...
void directionGraph::getready()
{
    ...
    if (visited!=nullptr) { MYfree(result); visited = nullptr; }
    if (before!=nullptr) { MYfree(result); before = nullptr; }
    if (distance!=nullptr) { MYfree(result); distance = nullptr; }
    if (distancelong!=nullptr) { MYfree(result); distancelong = nullptr; }
    if (path!=nullptr) { MYfree(result); path = nullptr; }
    if (branch!=nullptr) { MYfree(result); branch = nullptr; }
