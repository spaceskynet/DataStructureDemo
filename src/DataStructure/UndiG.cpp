...
void undirectionGraph::getready()
{
    ...
    if (weight!=nullptr) MYfree(weight);
    if (result!=nullptr) MYfree(result);
    if (visited!=nullptr) MYfree(visited);
    if (before!=nullptr) MYfree(before);
    if (distance!=nullptr) MYfree(distance);
    if (distancelong!=nullptr) MYfree(distancelong);
    if (path!=nullptr) MYfree(path);
    if (branch!=nullptr) MYfree(branch);
    if (startlong!=nullptr) MYfree(startlong);
    if (endlong!=nullptr) MYfree(endlong);
    if (indexlong!=nullptr) MYfree(indexlong);
...
void undirectionGraph::dijkstra(int inode)
{
    ...
    {
        visited[i] = 0;
        before[i] = weight[inode*vertexNum+i] < Infbound ?
            inode : -1;
        distance[i] = weight[inode*vertexNum+i] < Infbound ? 
            weight[inode*vertexNum+i] : Inf; 
        if (i == inode)
        {
            distance[i] = 0;
            visited[i] = 1;
...
void undirectionGraph::floyd()
{
    ...
    for(int i=0;i<vertexNum;i++)
	{
		for(int j=0;j<vertexNum;j++)
		{
			distancelong[i*vertexNum+j] = weight[i*vertexNum+j] < Infbound ? 
                weight[i*vertexNum+j] : Inf;
            path[i*vertexNum+j] = distancelong[i*vertexNum+j] < Infbound ?
                i : -1; //path from i to j, j's preceed. 
		}
	}
    ...
    for(int k=0;k<vertexNum;k++)//拿出每个顶点作为遍历条件
        for(int i=0;i<vertexNum;i++)
            for(int j=0;j<vertexNum;j++)
            {
                if( (distancelong[i*vertexNum+j]>distancelong[i*vertexNum+k]+distancelong[k*vertexNum+j]) 
                        && distancelong[i*vertexNum+k] < Infbound && distancelong[k*vertexNum+j] < Infbound)
                {
...
{
    if (!isready) getready();
    if (weight!=nullptr) { MYfree(weight); weight = nullptr; }
    if (result!=nullptr) { MYfree(result); result = nullptr; }
    if (visited!=nullptr) { MYfree(visited); visited = nullptr; }
    if (before!=nullptr) { MYfree(before); before = nullptr; }
    if (distance!=nullptr) { MYfree(distance); distance = nullptr; }
    if (distancelong!=nullptr) { MYfree(distancelong); distancelong = nullptr; }
    if (path!=nullptr) { MYfree(path); path = nullptr; }
    if (branch!=nullptr) { MYfree(branch); branch = nullptr; }
    if (startlong!=nullptr) { MYfree(startlong); startlong = nullptr; }
    if (endlong!=nullptr) { MYfree(endlong); endlong = nullptr; }
    if (indexlong!=nullptr) { MYfree(indexlong); indexlong = nullptr; }
