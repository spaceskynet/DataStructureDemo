...
void myArray::init()
{
    ...
    if (pArr != NULL)
        my_printf("数组对象内存分配成功\n");
    else
        my_printf("内存分配失败\n");
}

void myArray::initArray(int len)
{ //创建
    if (ready())
    {
...
bool myArray::insert(int pos, int val)
{ //插入元素
    if (!ready())
        return false;
    if (full())
